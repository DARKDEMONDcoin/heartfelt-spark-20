    ...
    <header className={cn("stripe-nav", variant === "over" && "is-over")} dir="rtl" onMouseLeave={() => setActive(null)} onKeyDown={(event) => { if (event.key === "Escape") setActive(null); }}>
      <nav className="stripe-nav-inner" aria-label="التنقل الرئيسي">
        ...
        <div className="stripe-nav-links">
          {groups.map((group) => <div key={group.label} onMouseEnter={() => setActive(group.label)}><Button type="button" variant="ghost" aria-expanded={active === group.label} aria-haspopup="menu" onClick={() => setActive(group.label)} onFocus={() => setActive(group.label)}>{group.label}<ChevronDown /></Button>{active === group.label && <div className="stripe-nav-popover" role="menu">{group.links.map((item) => <Link key={item.to} to={item.to} role="menuitem" onClick={() => setActive(null)}>{item.label}<span>←</span></Link>)}</div>}</div>)}
